...
def generate_stroke_mask(im_size, parts=4, maxVertex=20, maxLength=30, maxBrushWidth=15, maxAngle=360):
...
def generate_mask(type, im_size, mask_size):
    if type == 'rect':
        ...
        return generate_stroke_mask(im_size)


if __name__ == '__main__':
    a = generate_mask('other',(256,256),None)*255
    a = a.astype(np.uint8)
    print(a.shape)
    cv2.imshow('test',a[0][0])
    cv2.waitKey(1000000)