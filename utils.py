...
def get_window(in_tensor):
    bs,c,h,w = in_tensor.shape


def patchPartition(x):
    B,C,H,W = x.shape
    res = F.unfold(x,kernel_size=H//128,stride=1,padding=0)
    print(res.shape)
    res = res.permute(0, 2, 1).view(B, 4, C, H//2, W//2)
    return res

def patchMerge(x):
    B,N,C,H,W = x.shape
    res = x.view(B,N,C*H*W).permute(0,2,1)
    res = F.fold(res, kernel_size=H, stride=1, padding=0, output_size=(H*2, W*2))
    return res
...
def tensor2im(input_image, imtype=np.uint8):
    ...
    if not isinstance(input_image, np.ndarray):
        if isinstance(input_image, torch.Tensor):  # get the data from a variable
            ...
            return input_image
        image_numpy = image_tensor.cpu().float().clamp(-1.0, 1.0).numpy()  # convert it into a numpy array
...
def init_net(net, init_type='kaiming', init_gain=0.02, gpu_ids=[0], debug=False, initialize_weights=True):
    if len(gpu_ids) > 0:
